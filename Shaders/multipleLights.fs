...
vec3 calculateSpotLights(){
	...
	for(int i = 0; i < spotLightCount; i++){
		...
		//Esto es para manejar una atenuación (objetos lejanos se iluminan menos)
		result +=  intensity * attenuation * calculateDirectionalLight(spotLights[i].light, spotLights[i].direction);
		//result +=  intensity * calculateDirectionalLight(spotLights[i].light, spotLights[i].direction);///7
	}
	return result;
}
